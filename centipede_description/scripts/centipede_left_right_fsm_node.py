...
'''
# Name: centipede_left_right_fsm_node.py
...
def main():
    ...
    sis = smach_ros.IntrospectionServer('centipede_left_right_fsm_node' + str(rospy.get_name()), centipede_fsm_node, '/SM_ROOT') #+ str(rospy.get_name()))
    sis.start()

    # Give Gazebo some time to start up
    user_input = raw_input("Please press the 'Return/Enter' key to start executing - type: centipede_left_right_fsm_node.py | node: " + str(rospy.get_name()) + "\n")

    # Execute SMACH state machine
    print("Input received. Executing  - type: centipede_left_right_fsm_node.py | node: " + str(rospy.get_name()) + "...\n")
