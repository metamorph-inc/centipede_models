...
'''
# Name: centipede_sym_odd_even_fsm_node.py
...
def main():
    ...
    for i in range(num_segments):
        if i % 2 == 0:  # even segments -> StepLeft
            ...
            step_a_state_dict['seg' + str(i)] = Step(centipede_to_seg_dict['seg' + str(i)], "Recover",
                seg_to_centipede_dict['seg' + str(i)], "Ready", None, ros_rate, 'seg' + str(i))
    # Step B
    step_b_state_dict = {}
    for i in range(num_segments):
        if i % 2 == 0:  # even segments -> StepRight
            ...
            step_b_state_dict['seg' + str(i)] = Step(centipede_to_seg_dict['seg' + str(i)], "Plant",
                seg_to_centipede_dict['seg' + str(i)], "Ready", None, ros_rate, 'seg' + str(i))
    ...
    sis = smach_ros.IntrospectionServer('centipede_sym_odd_even_fsm_node' + str(rospy.get_name()), centipede_fsm_node, '/SM_ROOT') #+ str(rospy.get_name()))
    sis.start()

    # Give Gazebo some time to start up
    user_input = raw_input("Please press the 'Return/Enter' key to start executing - type: centipede_sym_odd_even_fsm_node.py | node: " + str(rospy.get_name()) + "\n")

    # Execute SMACH state machine
    print("Input received. Executing  - type: centipede_sym_odd_even_fsm_node.py | node: " + str(rospy.get_name()) + "...\n")
