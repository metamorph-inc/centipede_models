...
'''
# Name: centipede_front_back_fsm_node.py
...
def main():
    ...
    start_state_dict = {}
    for i in range(num_segments):
        if (i == 0) or (i == num_segments-1):  # front-back segments -> StartLeft (Init + StepRight)
    ...
    sis = smach_ros.IntrospectionServer('centipede_front_back_fsm_node' + str(rospy.get_name()), centipede_fsm_node, '/SM_ROOT') #+ str(rospy.get_name()))
    sis.start()

    # Give Gazebo some time to start up
    user_input = raw_input("Please press the 'Return/Enter' key to start executing - type: centipede_front_back_fsm_node.py | node: " + str(rospy.get_name()) + "\n")

    # Execute SMACH state machine
    print("Input received. Executing  - type: centipede_front_back_fsm_node.py | node: " + str(rospy.get_name()) + "...\n")
